...
class SystemConfigurationController extends Controller
{
    ...
    public function update(SystemConfiguration $systemConfiguration, Request $request): JsonResponse|RedirectResponse
    {
        $configurationValue = ConfigurationValue::findOrFail($systemConfiguration->id);
            ...
            'value' => ['required', 'string', function ($attribute, $value, $fail) use ($configurationValue) {
                if (str_contains("_range", $configurationValue->configuration->key)){
                    if (!$this->isValidRange($value, $configurationValue->configuration->key)) {
                        ...
                        } elseif ($configurationValue->configuration->key === 'shelf_range') {
                            $fail(__('validation.custom.value.range_numeric'));
                        }
