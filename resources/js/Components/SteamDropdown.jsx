...
const SteamDropdown = ({options, name, disabled = false, value, onChange, className = '', showEmptyValue = false}) => {
    return (
        <select
            name={name}
            disabled={disabled}
            ...
            value={value}>
                {showEmptyValue ? <option value=""></option> : <option value="">{StringHelper.Capitalize(StringHelper.__("Choose a value"))}</option> }
            {options.map(option => (
